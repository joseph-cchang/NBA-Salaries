...
The first principal component explains 70% of the variance in data, the next principal component explains 10.48%.
Having greater PPG is the best "indicator" for variance explained in data.
...
salary_prediction <- function(m, PPG){
  
  pre_new <- predict(m, PPG = points)
  
  msg <- as.formula(paste("PPG:", points,  " :Expected Salary: $", pre_new))
  print(paste(msg))
}

model <- lm(Salary ~ PPG, data = our_data)
salary_prediction(model, 16)
```
