...
This is where we merge the three datasets while also removing duplicated columns and columns that show unrelated/insignificant statistics. In addition, this unified dataset changes all columns (except Player and Salary) to dbl form for more convenient use later on. Small remark: a new column called Total_Minutes was added to see if it has any effect on data.
...
Based from the correlation, we will filter to only keep correlations with Salary above 0.6, meaning we will only use PPG, APG, MPG, TPG, Minutes_percent, free_throws_attempted, Offensive_Box_Plus_Minus, and Value_Over_Replacement.

From the 8 variables which have the highest correlation with salary, we have an interesting finding that the offensive statistics are more related to the salary than the defensive statistics. For example, points per game and assists per game are more correlated with salary than rebounds per game, steals per game, and blocks per game from the data above. In addition, offensive box plus minus and free throws attempted are also two important indicators of players' offensive ability. Hence, we may reach a conclusion that players with better offensive statistics are more likely to earn a higher salary than players with better defensive statistics. Furthermore, variables such as minutes per game, minutes percent, and turnovers per game suggest that when players are more involved in the game, they are more likely to earn a higher salary. Even though turnovers per game may sound like a negative indicator of a player's ability, it is not true. Only when a player is important for a team, can he make more turnovers on the court. If a player is not necessary for a team, he will be immediately benched by the coach. In other words, turnovers per game, minutes per game, and minutes percent are all indicators of a player's usage rate by the coach.

## Final dataset and Histograms

Our final dataset will be called our_data, and we will be using it the rest of the project. It has our players, their salaries, and the 8 predictors we want. To visualize this data, we used a histogram to plot Salary as well as scatter plots of Salary vs the 8 predictors.
...
From the scatterplots and the lines of best fits we can tell that salary tends to have a positive relationship with the 8 variables we choose above. In other words, the more time a player stays on the court and the better players' offensive abilities are, the higher the salary they will earn.
...
Our first model to test is linear regression. There were a total of 3 fitted models. The first model consists of all 8 predictors. The second fit contains predictors that summary of the first fit found significant. The third fit consists from predictors that the second fit found significant.
...
Based from the 3 fitted models, the largest R-squared came from fit1 at around 0.65. Next, I chose the model with the least MSE for training and testing. fit1 had the least MSE for training, but fit3 had the least MSE for testing.
...
We chose to do ridge & lasso because it eliminates overfitting through regularization. To start, I defined the training and testing sets for this process. In each training and testing, I had to split as x.train and y train as well as x.test and y.test. The x in front of the training and testing means all predictors except the response variable (Salary). The y signifies the response (Salary)
...
I first defined the list of lambda values the model can take, and used 5-folds cross-validation on the training data to predict the coefficients of each predictor.
...
Then, I found the mean squared error for this ridge regression model.
...
Next, I used cross-validation to choose the best tuning parameter (best lambda) and its plot
...
I printed out the coefficients of the best lambda. The glmnet function selects the minimum lambda and that lambda is used to predict the coefficients.
...
These coefficients represent the distance of each predictor to the response variable of Salary. I can determine that free_throws_attempted was the least and Value_Over_Replacement was highest. Since the MSE of testing for the best lambda value is less than the MSE of testing for the 5-folds CV, I will use the best lambda value MSE of testing which is around 7.172e+13.
...
In this step, I performed cross validation and MSE of testing data.
...
Just like for ridge, I can determine that turnovers_per_game was the least and Value_Over_Replacement was highest. The MSE of testing is around 7.171e+13, which is lower than the one given in the ridge regression.
...
Now here we do a prediction on the test set using the predict() command. We specify type='vector' because we are working with a regression decision tree. Then we calculate the mean squared error which is 9.12e+13 and root mean squared error which is 9549613.
...
We can now plot the pruned tree with 6 terminal nodes. The mean squared error (MSE) is lowest when the number of terminal nodes is 6.
...
The mean squared error is lower (8636943) than that of the non-pruned tree; the pruned tree with 6 terminal nodes is a better model.
...
Next, we used PCA to find the most influential predictors in the model. We first created dataset called dat that includes all predictors and found their variances.
...
Using Principle component Analysis, we found mean and standard deviation of the variables before PCA. There is also principal component loadings and a matrix of all principal component vectors (not shown in output due to excessive output)
...
I plotted the first and second prinicipal components to visualize the data. I then output the standard deviation and variance explained by each priniciple component from highest to lowest. Last, I found proportion of variance explained by each principal component from highest to lowest.
...
Based from the plot, the elbow point shows point 2, meaning I should consider the first two PCA because they are the largest 2 values. Therefore, the first principal component explains 70% of the variance in data, the next principal component explains 10.48%.

```{r}
pr.out$rotation[,1:2]
```

Based from the principal component loadings, the highest value in the first principal component is PPG, while the least is Value_over_Replacement. In the second PC, the highest is Offensive_Box_Plus_Minus and lowest is APG.

# Final Model Building

This is the final step: predicting a player's salary. We will go through the models and look for the best model to fit by finding lowest MSE. We determined that in the linear regression, both testing and training MSE were lower than the rest of the models. In order to determine which fit to use, we chose fit1 because it has the lowest training MSE and highest R-squared.
...
We test again for a player such as Nemanja Bjelica.

```{r}
our_data[which(our_data$Player == "Nemanja Bjelica"),]

salary_prediction(m = model, points = 7.2, assists = 1.9, minutes = 16.9, turnovers = 1.08, minutes_p = 35.2, free_throws = 42, offense_plus_minus = -1.4, value = -0.1)
```

Based on our prediction, his salary will be \$3,532,444, and his real salary is \$2,089,448.
...
After merging and filtering our dataset, we find out that the 8 variables which have the highest correlation with the players' salary are Points per game, Assists per game, minutes per game, turnovers per game, minutes percent, free-throws-attempted, offensive-box-plus-minus, and value_over_replacement. The result makes sense for us because it shows that the amount of time players spend on the court is highly correlated with their salary. However, we also find it interesting that the players' salary is weighted more towards their offensive statistics such as points per game and assists per game rather than their defensive statistics such as rebounds per game, steals per game, and blocks per game. In the sport of basketball, if a player can produce more on the offensive side, he will earn more money. 

After that, we started our analysis on NBA players' salary by plotting the histograms and the scatter plot of salary with the 8 variables listed above to have a first glance of the relationship between salary and the 8 variables we will analyze later in the project. Having some acknowledgement of our dataset, we reach an exploratory data analysis and split our data into a training dataset and testing dataset.

Next, we start to fit our data into different models: linear regression model, logistics regression model, Ridge / Lasso regression model, regression decision tree, random forest, and PCA. 

Here is a list of conclusions we developed from each model: 

[Linear regression:]{.underline} There were 3 fitted models. The first fit showed all predictors vs Salary, the second showed significant predictors from the first, and the third showed significant predictors from the second. We concluded that the largest R-squared came from fit1 at around 0.65. The fit1 also had the least training MSE at 3.409e+13, but fit3 had the least testing MSE at 7.304e+13.

[Logistic regression:]{.underline} Since all the eight variables we use are numeric variables, we create a new binary predictor called "Salarygreater". After we get the error rate of about 0.2 for both training and testing dataset, and the AUC of about 0.8226, we conclude that the eight variables we choose are good predictors of  the "Salarygreater" variables. 

[Ridge regression:]{.underline} We originally tested the lambda value on 5-folds cross validation, but found using the best tuning parameter was better at predicting coefficients because testing MSE for best lambda at 7.172e+13 is less than the testing MSE for the 5-folds CV at 7.175e+13. We determined coefficients for free_throws_attempted was the least at 9350 and Value_Over_Replacement was highest at 3025371. These numbers represent the distance of each predictor to the response variable of Salary. 

[Lasso regression:]{.underline} The least coefficient for lasso was turnovers_per_game at -882401 and the greatest was Value_Over_Replacement at 3852047. The MSE of testing is around 7.171e+13, which is lower than the one given in the ridge regression.

[Random Forest:]{.underline} we fit the random forest model again to the "Salarygreater" to get a error rate of about 0.2. In addition, according to the  Model Accuracy and Gini index, we find out that PPG is the most important variable, which corresponds to our finding in the linear regression method.

[Regression Decision Tree:]{.underline} We created a regression based decision tree based on “our_data”. This tree had 9 terminal nodes. We then fit the model to our training set and again the tree had 9 terminal nodes, or leaves.  The mean squared error for this tree was \$9549613. We then did a ten fold cross validation to see that the ideal number of leaves are 6 leaves. The variables used in this tree’s construction include “free_throws_attempted”, “PPG”, “Minutes_percent”, “Offensive_Box_Plus_Minus” and “MPG”. The mean squared error after pruning was \$8636943. We can see that the pruned tree is a better predictor because it has a lower mean squared error.

\

[PCA:]{.underline} After plotting the Proportion of Variance Explained and its cumulative values, we determined that the elbow point showed point 2, meaning we should consider the first two PCAs. Therefore, the first principal component explains 70% of the variance in data, the next principal component explains 10.48%. Based on the principal component loadings, the highest value in the first principal component is PPG at 0.3759, while the least is Value_over_Replacement at 0.3053. In the second PC, the highest is Offensive_Box_Plus_Minus at around 0.55 and lowest is APG at -0.32. These numbers show how much variance can be explained by the predictor.

The final step of this project is to predict the players' salary. As an example, we took Danny Green and Nemanja Bjelica's statistics to predict their salary. We can conclude that our model is somewhat accurate, as our predicted values differ from the actual by around only 1-2 million dollars. Overall, we can conclude that our model is only 65% accurate and still can be used to predict the majority of players' salaries correctly. 
...
We used these sites to find our data: <https://www.kaggle.com/umutalpaydn/nba-20202021-season-player-stats?select=nba2021_advanced.csv>
