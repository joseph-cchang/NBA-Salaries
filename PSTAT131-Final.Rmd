...
Here, we start by using the tree() function to create a decision tree. Then we take a summary of this tree to see variables used in tree construction, number of terminal nodes, and residual information.
...
```
The first split in this tree is on points per game. To be exact the split is on 14.06 points per game. The second splits are on minutes percent and free throws attempted. To be exact the split is on a minutes percent of 44.65 and 203 free throws attempted. The terminal nodes range from 7 observations to 143 observations and the total deviance explained is 69.1%.
...
To calculate the best number of terminal nodes we do a 10-fold cross validation. We plot size versus cross-validation error rate and add a vertical line at the minimum error. From this we can see 9 is the ideal number of terminal nodes because the cross-validation mis-classification error is the lowest at that point.
...
Next, we started to fit our data into different models: linear regression model, logistics regression model, Ridge / Lasso regression model, regression decision tree, and random forest.
