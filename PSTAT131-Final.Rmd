...
From the scatterplots and the regression lines we can tell that salary tends to have a positive relationship with the 8 variables we choose above. In other words, the more time players stay on the court and the better players' offensive abilities are, the higher salaries they will earn.
...
[Regression Decision Tree:]{.underline} 

We created a regression based decision tree based on "our_data". This tree had 9 terminal nodes. We then fit the model to our training set and again the tree had 9 terminal nodes, or leaves.  The mean squared error for this tree was \$9549613. We then did a ten fold cross validation to see that the ideal number of leaves are 6 leaves. The variables used in this tree's construction include "free_throws_attempted", "PPG", "Minutes_percent", "Offensive_Box_Plus_Minus" and "MPG". The mean squared error after pruning was \$8636943. We can see that the pruned tree is a better predictor because it has a lower mean squared error.\
