...
[Logistic regression:]{.ul} Since all the eight variables we use are numeric variables, we create a new binary predictor called "Salarygreater". After we get the error rate of about 0.2 for both training and testing dataset, and the AUC of about 0.8226, we conclude that the eight variables we choose are good predictors of  the "Salarygreater" variables. 
...
[Lasso regression:]{.ul} The least coefficient for lasso was turnovers_per_game at -882401 and the greatest was Value_Over_Replacement at 3852047. The MSE of testing is around 7.171e+13, which is lower than the one given in the ridge regression.

[Random Forest:]{.ul} we fit the random forest model again to the "Salarygreater" to get a error rate of about 0.2. In addition, according to the  Model Accuracy and Gini index, we find out that PPG is the most important variable, which corresponds to our finding in the linear regression method.

[Regression Decision Tree:]{.ul} 

\

[PCA:]{.ul} After plotting the Proportion of Variance Explained and its cumulative values, we determined that the elbow point showed point 2, meaning we should consider the first two PCAs. Therefore, the first principal component explains 70% of the variance in data, the next principal component explains 10.48%. Based on the principal component loadings, the highest value in the first principal component is PPG at 0.3759, while the least is Value_over_Replacement at 0.3053. In the second PC, the highest is Offensive_Box_Plus_Minus at around 0.55 and lowest is APG at -0.32. These numbers show how much variance can be explained by the predictor.
