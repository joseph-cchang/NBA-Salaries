...
From the result of the errors and AUC, we can say that the logistic regression works pretty well on the "Salarygreater" variable.
...
First, we will create a new column with High as a binary variable. It is "Yes" if the salary is greater than the median salary

```{r}
<<<<<<< HEAD
# create a new column with High as a binary variable. It is "Yes" if the salary is greater than the mean salary
=======

our_data <- our_data %>% mutate(High=as.factor(ifelse(Salary <= median(Salary), "No", "Yes")))
high.test = our_data$High
>>>>>>> 92fb7c1647ee1f02cbb6917797b6d47ae837e89a

# testing and training sets
training_high <- training %>% mutate(High=as.factor(ifelse(Salary <= mean(our_data$Salary), "No", "Yes")))
high.train = training_high$High

<<<<<<< HEAD
testing_high <- testing %>% mutate(High=as.factor(ifelse(Salary <= mean(our_data$Salary), "No", "Yes")))
high.test = testing_high$High
=======
```

Next, we will plot the fitted tree
...
tree_training_high = tree(High ~. -Salary, data = training_high) 
summary(tree_training_high)

# draw decision tree
draw.tree(tree_training_high, nodeinfo=TRUE, cex = 0.9)
title("Decision tree on training set", cex = 0.8)

<<<<<<< HEAD
# Predict on test set
tree.pred = predict(tree_training_high, testing_high, type="class")
=======
Then we will fit model on training set
...
```{r}
# see if pruning will lead to lower test error rate

# K=10-Fold cross validation
cv = cv.tree(tree_training_high, FUN= prune.misclass, K=5)
cv$size
cv$dev
...
plot(cv$size , cv$dev, type="b", xlab = "Number of leaves, \'best\'",
     ylab = "CV Misclassification Error", col = "black", main="CV")
...
```{r}
# Prune tree
pt.cv = prune.tree(tree_training_high, best=best.cv)
...
# confusion matrix
error.pt.cv <- table(pred.pt.cv, high.test)
error.pt.cv

1 - sum(diag(error.pt.cv))/sum(err.pt.cv)

# examine misclassification errors on training set

# print('training errors')
# classes_test <- as.data.frame(testing_high) %>% pull(High)
...
<<<<<<< HEAD
accuracy got worse after pruning
...
```{r}
rf_our_data=randomForest(salarygreater ~ PPG + APG + MPG + TPG + Minutes_percent + free_throws_attempted + Offensive_Box_Plus.Minus + Value_Over_Replacement, data = training_salarymean,mtry=3, ntree=500, importance=TRUE) 
rf_our_data
plot(rf_our_data)
```

Then we will calculate the tesing error rate.
```{r}
...
The testing error rat eis 0.1948.

Next, we will also get a plot with decreasing order of importance based on Model Accuracy and Gini value.

```{r}
varImpPlot(rf_our_data, sort=T, main="Variable Importance for rf_our_data", n.var=5)
```
The results indicate that across all of the trees considered in the random forest, PPG is by far the most important variable in terms of Model Accuracy and Gini index.
>>>>>>> 92fb7c1647ee1f02cbb6917797b6d47ae837e89a
