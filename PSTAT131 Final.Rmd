...
my_colnames <- c('Rank', 'Player', 'Team', 'Position', 'Age', 'Games.Played', 'MPG', 'Minutes%', 'Usage_Rate' , 'Turnover rate', 'free throws attempted', 'Free throw%', '2-point field goals attempted', '2-point%', '3-point field goals attempted', 'three-point %', 'effective shooting %' , 'True shooting %', 'PPG', 'RPG', 'Total rebound %', 'APG', 'Assist %', 'SPG' ,'BPG', 'TPG', 'Versatility_Index', 'Offensive Rating' , 'Defensive rating')
...
```{r}
salarycor <- our_data %>% select(Salary,Games.Played,PPG,RPG,APG,SPG,BPG,TPG,MPG,`Usage Rate`,`Free throw%`,`three-point %`,`Versatility Index`,`Offensive Rating`,`Defensive rating`,win.shares,Box.Plus.Minus,Value.Over.Replacement) %>% drop_na()
#ggpairs(salarycor)
cor(salarycor)[,"Salary"]
```

Based from the correlation, we see that PPG>MPG>TPG>Value.Over.Replacement>APG>win.shares>Versatility Index>Box.Plus.Minus>
Usage Rate>RPG>SPG>Free throw%>Offensive Rating>Defensive rating>BPG>Games.Played>three-point %
...
# training data
fit <- lm(Salary ~ PPG + MPG+TPG+Value.Over.Replacement+APG+win.shares+ Versatility_Index+ Box.Plus.Minus + Usage_Rate, data = training)
summary(fit)
...
fit2 <- lm(Salary ~ PPG + Usage_Rate + APG, data= fit_data)
summary(fit2)
...
set.seed(2252022)
dat <- model.matrix(Salary~., fit_data) 
train = sample(nrow(dat))
x.train = dat[train, ]
y.train = fit_data[train, ]$Salary

# The rest as test data
x.test = dat[-train, ]
y.test = fit_data[-train, ]$Salary
```
...
ridge.pred=predict(ridge.mod, s = ridge.mod$lambda.min, type="coefficients")
ridge.pred

# lasso
lambda.list.lasso = 2 * exp(seq(0, log(1e-4), length = 100))
lasso.mod <- cv.glmnet(x.train, y.train, alpha=1, lambda=lambda.list.lasso, nfolds = 10)
lasso.pred = predict(lasso.mod, s = lasso.mod$lambda.min, type="coefficients")
lasso.pred
...
```{r}
# ridge MSE
training_pred <- predict(ridge.mod, s = ridge.mod$lambda.min, newx = x.train)
mean((training_pred - y.train)^2)

testing_pred <- predict(ridge.mod, s = ridge.mod$lambda.min, newx = x.test)
mean((testing_pred - y.test)^2)

# lasso MSE
training_predict <- predict(lasso.mod, s = lasso.mod$lambda.min, newx = x.train)
mean((training_predict - y.train)^2)

testing_predict <- predict(lasso.mod, s = lasso.mod$lambda.min, newx = x.test)
mean((testing_predict - y.test)^2)
```
