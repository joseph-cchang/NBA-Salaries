...
salaries <- read.csv("nba_salaries_21-22.csv", col.names = labels2, na = "XXX")
salaries
```
...
selection <- subset(combine, select= -c(2:4,7,9,10,12:14,20,22,29:32,35:47,49:51,54,56:57))
...
conversion <- less_data[,2:23] %>% mutate_if(is.character,as.numeric)
...
our_data1 <- conversion2 %>% filter(Games.Played >= 10) %>% select(Player:Salary)
our_data2 <- our_data1 %>% filter(MPG > 10) %>% select(Player:Salary)
our_data2
```
...
my_cor <- our_data2 %>% select_if(is.numeric) %>% drop_na() %>% cor() %>% round(3)
corrplot(my_cor, method = "circle", type = "upper")
```
##Correlation2
```{r}
salarycor<- our_data2 %>% select(Salary, Games.Played,PPG,RPG,APG,SPG,BPG,TPG,Minutes.played,MPG,`Usage Rate`,`Free throw%`,`three-point %`,`Versatility Index`,`Offensive Rating`,`Defensive rating`,win.shares,Box.Plus.Minus,Value.Over.Replacement) %>% drop_na()
ggpairs(salarycor)
...
```{r}
plot(our_data2$Salary)
hist(our_data2$Salary)
```
...
fit_data <- our_data2[-1]
fit <- lm(Salary ~ ., data = fit_data)
summary(fit)
...
new_data <- data %>% resample_partition(p = c(test=0.2, train=0.8))
training <- new_data$train
testing <- new_data$test
```
